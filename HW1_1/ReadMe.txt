...
應用程式精靈已經為您建立了這個 HW1_1 應用程式。這個應用程式不僅示範了使用 MFC 的基本概念，也是您撰寫應用程式的起點。
...
HW1_1.h
    這是應用程式的主要標頭檔。
    它包含了特定標頭 (包括 Resource.h)，並宣告 CHW1_1App 應用程式類別。

HW1_1.cpp
    這是含有應用程式類別 CHW1_1App 的主要應用程式原始程式檔。

HW1_1.rc
    這是程式所用的所有 Microsoft Windows 資源的列表。它含有儲存在 RES 子目錄中的圖示、點陣圖和游標。您可以直接在 Microsoft Visual C++ 內編輯這個檔案。您的專案資源是在 1028。

res\HW1_1.ico
    這是用來做為應用程式圖示的圖示檔。這個圖示是包含在主要資源檔 HW1_1.rc 中。
...
HW1_1Dlg.h、HW1_1Dlg.cpp - 對話方塊
    這些檔案包含您的 CHW1_1Dlg 類別。這個類別會定義應用程式主對話方塊的行為。對話方塊的範本是在可於 Microsoft Visual C++ 編輯的 HW1_1.rc 中。
